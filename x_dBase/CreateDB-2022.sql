-- SQL Server 2022 version of the PokerApp database creation script.
-- Requires SQL Server 2022 or later due to STRING_SPLIT(..., 1) ordinal support.
-- Modernizes parsing and DDL while preserving the database contract used by the app.
...
/************************************************************************************
    ...
    Target Platform:
    - Optimized for SQL Server 2022 
      leveraging STRING_SPLIT(@s, '|', 1) with ordinal output.

    Notes:
    - Implements native STRING_SPLIT(@s, '|', 1) for ordered, pipe-delimited parsing.
    - Maintains transaction safety (SET XACT_ABORT ON) in write paths.
    - Uses datetime2/sysdatetime() defaults and identity keys consistent with 2022 guidance.
    - Includes a RAW deck accessor for app-side shuffle.

    Legacy Notice:
    - [dbo].[GameDeck_GetShuffledDeck] is retained for backward compatibility only.
      Shuffle is now performed in the C# Web Application.
      Use [dbo].[GameDeck_GetRawDeck] for current integrations.

    Compatibility:
    - This script is not intended for SQL Server versions prior to 2022.
      For SQL Server 2008–2019, use CreateDB-2019.sql (DelimitedSplit8K).
************************************************************************************/
...
Begin Tran
	...
	From STRING_SPLIT(@Array, '|', 1) AS s
	Order By s.[ordinal]
